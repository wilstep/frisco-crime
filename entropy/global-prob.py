...
sumt = pdf.sum()
pdf = np.divide(pdf, sumt) # normalise probabilities
S = 0.0 # we will compute the entropy and store it here
for i in range(0, len(pdf)):
   S -= pdf[i] * math.log(pdf[i])
print "Entropy S = %f" % S
...
with open('global.csv','wb') as f_handle:
